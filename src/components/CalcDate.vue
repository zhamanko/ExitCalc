...
function calculateDate(dateVal, daysVal) {
  if (!dateVal || !daysVal || isNaN(daysVal)) {
    return '――'
  }

  const start = new Date(dateVal)
  if (isNaN(start.getTime()) || daysVal < 1) {
    return '――'
  }

  const strDay = ['Неділя', 'Понеділок', 'Вівторок', 'Середа', 'Четвер', "П'ятниця", 'Субота']

  start.setDate(start.getDate() + (Number(daysVal) - 1))
...
<template>
  <div class="bg-zinc-800 p-6 inline-block w-full text-white shadow-[10px_10px_0_rgba(0,0,0,0.7)]">
    ...
    <div class="flex gap-2 flex-col mt-4">
      <label for="days">Календарні дні</label>
      <input
        id="days"
        type="number"
        v-model.number="days"
        placeholder="Введіть календарні дні"
        class="w-full p-2 bg-[#212121]"
      />
    </div>
